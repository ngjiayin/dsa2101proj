...
Our group has chosen to use data from the ‘inventory_parts’,’inventories’,’sets’ and ‘colors’ dataset to come up with the three visualisations in the following portion. In part_colors,  left_join() was used to join the first inventory_parts and colors datasets using their color_id and subsequently used the select() to omit columns which were redundant and would not be required in the visualisations. The similar process was done to obtain part_colors_set but this time the left_join was used to join part_colors, with the inventories dataset, and finally another left_join() once more  joining  part_colors_set to the sets dataset.
...
ggplot(color_theme_life_span, aes(x = name, y = start_year)) +
  ...
  coord_flip() +
  labs(title = "Consistency of Colours in LEGO sets",
      subtitle = "Life span of all colours from 1949 to 2022",
       x = "Colours", y = "Years") +
  theme_minimal() +
